...
public class BinarySearch {
    ...
    public static int search(int[] arr, int key) {
        ...
        while (start <= end) {
            // calculate the middle element for the current change
            int mid = start + (end - start) / 2;
            if (key == arr[mid])
                return mid;
            if (isAscending) { // ascending order
                if (key < arr[mid]) {
                    end = mid - 1; // the key can be in first half
                } else { // key > arr[mid]
                    start = mid + 1; // the key present in the second half
                }
            } else {
                if (key > arr[mid]) { // descending order
                    end = mid - 1;
                } else {
                    start = mid + 1;
                }
