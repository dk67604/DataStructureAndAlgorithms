...
public class AlienDictionary {
    public static String findOrder(String[] words) {
        if (words == null || words.length == 0)
            return "";
        // a. Initialize graph
        Map<Character,Integer> inDegree = new HashMap<>(); // count of incoming edges for every vertex
        Map<Character, List<Character>> graph = new HashMap<>(); // adjacency list graph
        ...
        for(int i =0;i < words.length - 1; i++){
            ...
            for(int j =0; j < Math.min(w1.length(),w2.length()); j++){
                    ...
                    if(parent != child) { // if the two character are different
